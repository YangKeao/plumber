...
use crate::parser::*;
use self::llvm::{LLVMContext, LLVMModule, LLVMBuilder, LLVMValue};
use self::llvm::prelude::LLVMValueRef;
use std::collections::HashMap;
use self::llvm::core::{LLVMBuildAdd, LLVMGetParam, LLVMConstInt, LLVMGetNamedFunction, LLVMBuildCall, LLVMBuildSub, LLVMBuildMul, LLVMAddTargetDependentFunctionAttr};
use self::llvm::execution_engine::LLVMRecompileAndRelinkFunction;

pub trait IrGen {
    fn build(&self, ctx: *mut LLVMContext, module: *mut LLVMModule, builder: *mut LLVMBuilder, value_map: &HashMap<&str, LLVMValueRef>) -> Option<LLVMValueRef>;
}
    ...
    fn build(&self, ctx: *mut LLVMContext, module: *mut LLVMModule, builder: *mut LLVMBuilder, value_map: &HashMap<&str, LLVMValueRef>) -> Option<LLVMValueRef> {
        ...
        match self {
            MonadicExpression::FunctionCall(fun_call) => {
                let function = unsafe {
                    LLVMGetNamedFunction(module, format!("{}\0", fun_call.function_name.get_name()).as_ptr() as *const _)
                };
                let mut args: Vec<_> = fun_call.args.iter().map(|item| {
                    item.build(ctx, module, builder, value_map).unwrap()
                }).collect();
                Some(unsafe {
                    // TODO: set LLVMBuildCall Name
                    LLVMBuildCall(builder, function, args.as_mut_ptr(), args.len() as u32, format!("{}\0", fun_call.function_name.get_name()).as_ptr() as *const _)
                })
            },
...
impl IrGen for BinaryExpression {
    fn build(&self, ctx: *mut LLVMContext, module: *mut LLVMModule, builder: *mut LLVMBuilder, value_map: &HashMap<&str, LLVMValueRef>) -> Option<LLVMValueRef> {
        ...
        match self.op {
            BinaryOperation::Plus => Some(unsafe {LLVMBuildAdd(builder, left, right, b"addtmp\0".as_ptr() as *const _)}),
            BinaryOperation::Minus => Some(unsafe {LLVMBuildSub(builder, left, right, b"subtmp\0".as_ptr() as *const _)}),
            BinaryOperation::Mul => Some(unsafe {LLVMBuildMul(builder, left, right, b"multmp\0".as_ptr() as *const _)}),
            _ => unimplemented!()
        }
    }
}
...
impl IrGen for Expression {
    fn build(&self, ctx: *mut LLVMContext, module: *mut LLVMModule, builder: *mut LLVMBuilder, value_map: &HashMap<&str, LLVMValueRef>) -> Option<LLVMValueRef> {
        match self {
            Expression::BinaryExpression(binary_expr) => binary_expr.build(ctx, module, builder, value_map),
            Expression::MonadicExpression(monadic_expr) => monadic_expr.build(ctx, module, builder, value_map),
        }
    }
}
...
impl IrGen for FunDefinition {
    fn build(&self, ctx: *mut LLVMContext, module: *mut LLVMModule, builder: *mut LLVMBuilder, value_map: &HashMap<&str, LLVMValueRef>) -> Option<LLVMValueRef> {
        unsafe {
            ...
            let function_type = llvm::core::LLVMFunctionType(int, params.as_mut_ptr(), self.args.len() as u32, 0);
            let function =
            ...
            llvm::core::LLVMBuildRet(builder, self.expr.build(ctx, module, builder, &bind_map).unwrap());
        }
        None
    }
}
